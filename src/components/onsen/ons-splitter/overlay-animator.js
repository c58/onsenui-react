...
export default class OverlaySplitterAnimator extends SplitterAnimator {
  ...
  layoutOnClose() {
    animit(this._side.element)
      .queue({
        transform: 'translateX(0%)',
        width: this._side.getWidth(),
      })
      .play();
  ...
  layoutOnOpen() {
    animit(this._side.element)
      .queue({
        transform: 'translate3d(' + (this._side.isLeft ? '' : '-') + '100%, 0px, 0px)',
        width: this._side.getWidth(),
      })
      .play();
  ...
  activate(sideObj) {
    ...
    this._content = sideObj.getContentElement();
    this._side = sideObj;
    this._mask = sideObj.getMaskElement();
    this._setupLayout();
  }
  ...
  translate(distance) {
    animit(this._side.element)
      .queue({
        transform: 'translate3d(' + (this._side.isLeft ? '' : '-') + distance + 'px, 0px, 0px)'
      })
      .play();
  }

  _clearLayout() {
    const side = this._side.element;
   ...
   */
  open(done) {
    const transform = this._side.isLeft ? 'translate3d(100%, 0px, 0px)' : 'translate3d(-100%, 0px, 0px)';
    const side = this._side.element;
    const mask = this._mask;
